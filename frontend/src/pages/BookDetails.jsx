...
const BookDetails = () => {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto w-full p-6 lg:p-8 rounded-[2rem] glass-light mt-4 flex flex-col lg:flex-row gap-10">
        
        {/* Left Side: Image Gallery Area */}
        <div className="w-full lg:w-1/2 flex flex-col gap-4">
          <div className="w-full aspect-[4/3] rounded-[1.5rem] overflow-hidden bg-black/10 border border-gray-200">
            <img src={imageUrl} alt={listing.title} className="w-full h-full object-cover" />
          </div>
          {listing.images?.length > 1 && (
            <div className="flex gap-4">
              {listing.images.slice(1).map((img, idx) => (
                <img key={idx} src={`http://localhost:5000${img}`} className="w-24 h-24 rounded-xl object-cover cursor-pointer hover:opacity-80 border border-gray-200" alt={`View ${idx + 2}`} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xl text-gray-500 mb-6">by {listing.author}</p>

            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="p-4 rounded-2xl bg-white/50 border border-gray-200">
                <p className="text-sm text-gray-500 mb-1">Condition</p>
                <p className="font-bold text-lg">{listing.condition}</p>
              </div>
              <div className="p-4 rounded-2xl bg-white/50 border border-gray-200">
                <p className="text-sm text-gray-500 mb-1">Edition</p>
                <p className="font-bold text-lg">{listing.edition || 'N/A'}</p>
              </div>
            </div>

            <div className="p-5 rounded-2xl bg-black/5 border border-gray-200 flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{listing.seller.college}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-end justify-between border-t border-gray-200 pt-6">
            ...
            {!isOwner ? (
              ...
              <div className="px-6 py-3 font-bold rounded-xl bg-gray-200 text-gray-500 cursor-not-allowed">
                Your Listing
              </div>
            )}
          </div>
